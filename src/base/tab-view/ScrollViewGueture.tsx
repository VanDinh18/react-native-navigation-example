...
export function dealWithAnimation(
  ...
  switch (withAnimation.type) {
    case 'spring':
      return (value, cb) =>
        withSpring(value, withAnimation.config, isFinished =>
          cb(isFinished as boolean),
        );
    case 'timing':
      return (value, cb) =>
        withTiming(value, withAnimation.config, isFinished =>
          cb(isFinished as boolean),
        );
  }
}

const IScrollViewGesture = (props: Props) => {
  //   const {
  //     vertical,
  //     pagingEnabled,
  //     snapEnabled,
  ...
  const [overscrollEnabled] = useState(true);
  ...
  const withSpring = React.useCallback(
    (toValue: number, onFinished?: () => void) => {
      ...
      return dealWithAnimation(defaultWithAnimation)(
        toValue,
        (isFinished: boolean) => {
          'worklet';
